...
class CriteoDataset(torch.utils.data.Dataset):
    ...
    def __read_train_all_feats(self):
        ...
        return pickle.load(open("train_all_feat.pkl811", "rb"))
    ...
    def __get_feat_mapper(self, path):
        ...
        f = open("train_all_feat811.pkl", "wb")
...
def get_criteo_dataset_loader(train_path="criteo_train_4000w.txt", test_path="criteo_test_500w.txt", batch_size=256):
    ...
    train_dataset = CriteoDataset(dataset_path=train_path, cache_path=".criteo_train2",rebuild_cache=False)
    print("开始加载测试集合")
    test_dataset = CriteoDataset(dataset_path=test_path, cache_path=".criteo_test2",rebuild_cache=False)
...
def get_criteo_dataset_train(train_path="train.txt", batch_size=128):
    # the test_path maybe null, if it is, we need to split the train dataset
